...
import {
  salesOrders, purchaseOrders, products, warehouses, customers, vendors,
...
export class WorkflowService {
  ...
  async confirmSalesOrder(salesOrderId: string, userId: string): Promise<{
    ...
    const salesOrderLines = await this.getSalesOrderLines(salesOrderId);
    ...
    const [updatedSalesOrder] = await db.update(salesOrders)
  ...
  async createDeliveryFromSalesOrder(salesOrderId: string, userId: string): Promise<{
    ...
    const salesOrderLines = await this.getSalesOrderLines(salesOrderId);
    const deliveryNumber = await this.getNextDocumentNumber(salesOrder.companyId, 'DEL');
    ...
    for (let i = 0; i < salesOrderLines.length; i++) {
      ...
      const movementNumber = await this.getNextDocumentNumber(salesOrder.companyId, 'SM');
      ...
      const [deliveryLine] = await db.insert(deliveryLines).values({
        id: deliveryLineId,
        deliveryId,
        lineNumber: i + 1,
        productId: line.productId,
      ...
      await db.update(stockReservations)
        ...
        .where(and(
          ...
          eq(stockReservations.productId, line.productId)
        ));
    }

    let journalEntry: JournalEntry | null = null;
    if (totalCOGS > 0) {
      journalEntry = await this.createCOGSJournalEntry(
        salesOrder.companyId,
        deliveryId,
        deliveryNumber,
        totalCOGS,
  ...
  async createInvoiceFromSalesOrder(salesOrderId: string, userId: string): Promise<{
    ...
    const salesOrderLines = await this.getSalesOrderLines(salesOrderId);
    const invoiceNumber = await this.getNextDocumentNumber(salesOrder.companyId, 'INV');
    ...
    for (let i = 0; i < salesOrderLines.length; i++) {
      ...
      const [invoiceLine] = await db.insert(invoiceLines).values({
        id: invoiceLineId,
        invoiceId,
        lineNumber: i + 1,
        productId: line.productId,
    ...
    const journalEntry = await this.createARInvoiceJournalEntry(
      salesOrder.companyId,
      invoiceId,
      invoiceNumber,
      total,
    ...
    const arLedgerEntry = await this.createARLedgerEntry(
      salesOrder.companyId,
  ...
  async receivePayment(
    ...
    const paymentNumber = await this.getNextDocumentNumber(companyId, 'PAY');
    ...
    const journalEntry = await this.createPaymentReceiptJournalEntry(
      companyId,
      paymentId,
      paymentNumber,
      amount,
    ...
    const arLedgerEntry = await this.createARPaymentLedgerEntry(
      companyId,
      customerId,
      paymentId,
      paymentNumber,
  ...
  async confirmPurchaseOrder(purchaseOrderId: string, userId: string): Promise<PurchaseOrder> {
    ...
    const purchaseOrderLines = await this.getPurchaseOrderLines(purchaseOrderId);
    ...
    const [updatedPurchaseOrder] = await db.update(purchaseOrders)
  ...
  async receiveGoodsFromPurchaseOrder(purchaseOrderId: string, userId: string): Promise<{
    ...
    const purchaseOrderLines = await this.getPurchaseOrderLines(purchaseOrderId);
    const receiptNumber = await this.getNextDocumentNumber(purchaseOrder.companyId, 'GR');

    const receiptId = randomUUID();
    const [goodsReceipt] = await db.insert(goodsReceipts).values({
      id: receiptId,
    ...
    for (let i = 0; i < purchaseOrderLines.length; i++) {
      ...
      const movementNumber = await this.getNextDocumentNumber(purchaseOrder.companyId, 'SM');
      const movementId = randomUUID();
      const [stockMovement] = await db.insert(stockMovements).values({
        id: movementId,
        companyId: purchaseOrder.companyId,
      ...
      const [receiptLine] = await db.insert(goodsReceiptLines).values({
        id: receiptLineId,
    ...
    const journalEntry = await this.createGoodsReceiptJournalEntry(
      purchaseOrder.companyId,
      receiptId,
      receiptNumber,
      totalCost,
  ...
  async createVendorInvoice(purchaseOrderId: string, userId: string): Promise<{
    ...
    const [goodsReceipt] = await db.select().from(goodsReceipts)
      .where(eq(goodsReceipts.purchaseOrderId, purchaseOrderId));

    const purchaseOrderLines = await this.getPurchaseOrderLines(purchaseOrderId);
    const invoiceNumber = await this.getNextDocumentNumber(purchaseOrder.companyId, 'INV');
    ...
    const journalEntry = await this.createAPInvoiceJournalEntry(
      purchaseOrder.companyId,
      invoiceId,
      invoiceNumber,
      total,
    ...
    const apLedgerEntry = await this.createAPLedgerEntry(
      purchaseOrder.companyId,
  ...
  async makeVendorPayment(
    ...
    const journalEntry = await this.createVendorPaymentJournalEntry(
      companyId,
      paymentId,
      paymentNumber,
      amount,
    ...
    const apLedgerEntry = await this.createAPPaymentLedgerEntry(
      companyId,
      vendorId,
      paymentId,
      paymentNumber,
